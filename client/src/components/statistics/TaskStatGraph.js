...
export default function TaskStatGraph(){
    ...
    async function getTaskStats(){
            ...
            let statDataReq = await axios.get(`http://localhost:8282/taskStat/${current}/`)
            let {entries, averageDuration, netTime, timesCompleted} = statDataReq.data
            let tempDates = []
            let tempDurations = []
    ...
    return(
        <GraphContainer>
            <form>
                ...
                <select onChange={(e) => handleChange(e)} value={current} id="task"name="task">
                    {tasks.map((task) =>{
                        return <option id={task.name} value={task._id}>{task.name}</option>
                    })}
                </select>
            </form>
